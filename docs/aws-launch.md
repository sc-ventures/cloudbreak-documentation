...
The minimum set of permissions required by Cloudbreak are the same as for the [CredentialRole](#create-credentialrole). 
...
Before launching Cloudbreak on AWS, review and meet the AWS-specific [prerequisites](#prerequisites). Next, follow the steps below.  
...
As part of the prerequisites, you had two options to allow Cloudbreak to authenticate with AWS and create resources on your behalf: key-based or role-based [authentication](aws-launch.md#authentication). Depending on your choice, you must configure a key-based or role-based credential: 
...
To perform these steps, you must know the **IAM Role ARN** corresponding to the "[CredentialRole](#create-credentialrole)" (configured as part of the prerequisites).   
...
**Related links**  
[Create CredentialRole](#create-credentialrole)        
